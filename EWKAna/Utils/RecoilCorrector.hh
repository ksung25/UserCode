...
/*
  ...
  // Model for PF u1
  //
  Double_t pfu1mean   = fcnPFu1mean->Eval(inPt);
  Double_t pfu1sigma1 = fcnPFu1sigma1->Eval(inPt);
  Double_t pfu1sigma2 = fcnPFu1sigma2->Eval(inPt);
  Double_t pfu1sigma0 = fcnPFu1sigma0->Eval(inPt);
  if(nsigma!=0) {
    //pfu1mean   += nsigma*dMean(fcnPFu1mean,inPt,fitresPFu1mean);
    pfu1sigma1 += nsigma*dSigma(fcnPFu1sigma1,inPt,fitresPFu1sigma1);
    pfu1sigma2 += nsigma*dSigma(fcnPFu1sigma2,inPt,fitresPFu1sigma2);
    pfu1sigma0 += nsigma*dSigma(fcnPFu1sigma0,inPt,fitresPFu1sigma0);
  }
  ...
  // Model for PF u2
  //
  Double_t pfu2mean   = fcnPFu2mean->Eval(inPt);
  Double_t pfu2sigma1 = fcnPFu2sigma1->Eval(inPt);
  Double_t pfu2sigma2 = fcnPFu2sigma2->Eval(inPt);
  Double_t pfu2sigma0 = fcnPFu2sigma0->Eval(inPt);
  if(nsigma!=0) {
    //pfu2mean   += nsigma*dMean(fcnPFu2mean,inPt,fitresPFu2mean);
    pfu2sigma1 += nsigma*dSigma(fcnPFu2sigma1,inPt,fitresPFu2sigma1);
    pfu2sigma2 += nsigma*dSigma(fcnPFu2sigma2,inPt,fitresPFu2sigma2);
    pfu2sigma0 += nsigma*dSigma(fcnPFu2sigma0,inPt,fitresPFu2sigma0);
  }
  Double_t pfu2frac2  = (pfu2sigma0 - pfu2sigma1)/(pfu2sigma2 - pfu2sigma1);

  Double_t z1 = gRandom->Gaus(0,1);
  Double_t z2 = gRandom->Gaus(0,1);
  Double_t p1 = gRandom->Gaus(0,1);
  Double_t p2 = gRandom->Gaus(0,1);
*/
/*
  Double_t pfn1  = sqrt(1.0 + pfc*pfc + u1c*u1c + sqrt(TMath::Pi())*(pfc+u1c));
  Double_t pfn2  = sqrt(1.0 + pfc*pfc + u2c*u2c + sqrt(TMath::Pi())*(pfc+u2c));

  Double_t pfu1 = (gRandom->Uniform(0,1) < pfu1frac2) ? 
                  (1.+pfc*fabs(z2)+u1c*fabs(p1))/pfn1*pfu1sigma2*z1+pfu1mean : 
		  (1.+pfc*fabs(z2)+u1c*fabs(p1))/pfn1*pfu1sigma1*z1+pfu1mean;
  
  Double_t pfu2 = (gRandom->Uniform(0,1) < pfu2frac2) ? 
                  (1.+pfc*fabs(z1)+u2c*fabs(p2))/pfn2*pfu2sigma2*z2+pfu2mean : 
		  (1.+pfc*fabs(z1)+u2c*fabs(p2))/pfn2*pfu2sigma1*z2+pfu2mean;

  TVector2 vpfmet(-(dilepPt+pfu1)*cos(dilepPhi)+pfu2*sin(dilepPhi), -(dilepPt+pfu1)*sin(dilepPhi)-pfu2*cos(dilepPhi));  
  pfmet    = vpfmet.Mod();
  pfmetphi = TVector2::Phi_mpi_pi(vpfmet.Phi());  
//*/
/*  
  Double_t pfc1  = hCorrPFu2u1->GetBinContent(ibin);
  Double_t pfc2  = hCorrPFu1u2->GetBinContent(ibin);
  
  Double_t pfn1  = sqrt(1.0 + pfc1*pfc1 + u1c*u1c + sqrt(TMath::Pi())*(pfc1+u1c));
  Double_t pfn2  = sqrt(1.0 + pfc2*pfc2 + u2c*u2c + sqrt(TMath::Pi())*(pfc2+u2c));
  
  Double_t pfu1 = (gRandom->Uniform(0,1) < pfu1frac2) ? 
                  (1.+pfc1*fabs(z2)+u1c*fabs(p1))/pfn1*pfu1sigma2*z1+pfu1mean : 
		  (1.+pfc1*fabs(z2)+u1c*fabs(p1))/pfn1*pfu1sigma1*z1+pfu1mean;
  
  Double_t pfu2 = (gRandom->Uniform(0,1) < pfu2frac2) ? 
                  (1.+pfc2*fabs(z1)+u2c*fabs(p2))/pfn2*pfu2sigma2*z2+pfu2mean : 
		  (1.+pfc2*fabs(z1)+u2c*fabs(p2))/pfn2*pfu2sigma1*z2+pfu2mean;
...
/*  
  ...
  // 
  Double_t pfc1 = hCorrPFu2u1->GetBinContent(ibin);
  Double_t pfn1 = sqrt(1.0 + pfc1*pfc1 + sqrt(TMath::Pi())*pfc1);
  Double_t pfc2 = hCorrPFu1u2->GetBinContent(ibin);
  Double_t pfn2 = sqrt(1.0 + pfc2*pfc2 + sqrt(TMath::Pi())*pfc2);
  Double_t pfu1 = (gRandom->Uniform(0,1) < pfu1frac2) ? (1.+pfc1*fabs(z2))/pfn1*pfu1sigma2*z1+pfu1mean : (1.+pfc1*fabs(z2))/pfn1*pfu1sigma1*z1+pfu1mean;
  Double_t pfu2 = (gRandom->Uniform(0,1) < pfu2frac2) ? (1.+pfc2*fabs(z1))/pfn2*pfu2sigma2*z2+pfu2mean : (1.+pfc2*fabs(z1))/pfn2*pfu2sigma1*z2+pfu2mean;
