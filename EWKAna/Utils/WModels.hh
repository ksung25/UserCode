...
CPepeModel1::CPepeModel1(const char *name, RooRealVar &x)
{
  ...
  // f(x) = x*exp[-x^2 / (s + ax)^2] = x*exp[-x*x/(s*s + 2*a*x + x*x)]
  char formula[200];
  sprintf(formula,
...
CPepeModel2::CPepeModel2(const char *name, RooRealVar &x)
{
  char sigmaName[50]; sprintf(sigmaName,"sigma_%s",name); sigma = new RooRealVar(sigmaName,sigmaName,10,5,20);
  char a1Name[50];    sprintf(a1Name,   "a1_%s",   name); a1    = new RooRealVar(a1Name,a1Name,0.1,-1,1);
  char a2Name[50];    sprintf(a2Name,   "a2_%s",   name); a2    = new RooRealVar(a2Name,a2Name,0.1,-1,1);
  
  // f(x) = x*exp[-x^2 / (s + ax + bx^2)^2] = x*exp[-x*x/(s + a*x + b*x*x)/(s + a*x + b*x*x)]
  char formula[300];
  sprintf(formula,
          "%s*exp(-%s*%s/(%s + %s*%s + %s*%s*%s)/(%s + %s*%s + %s*%s*%s))",
	  x.GetName(),
	  x.GetName(),x.GetName(),
	  sigmaName,a1Name,x.GetName(),a2Name,x.GetName(),x.GetName(),
	  sigmaName,a1Name,x.GetName(),a2Name,x.GetName(),x.GetName());
