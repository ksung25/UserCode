...
echo `hostname`
echo "Running runSCNtupler.C $FILESET $DATASET $BOOK $CATALOG $ISDATA $USEGEN $FSRMODE $NEVENTS $SKIPHLTFAIL"
echo "Working in $WORKDIR"
ls -l
echo "   Using $CMSSW_BASE"

root -b -l -q ./rootlogon.C runSCNtupler.C+\(\"${FILESET}\",\"${DATASET}\",\"${BOOK}\",\"${CATALOG}\",${ISDATA},${USEGEN},${FSRMODE},${NEVENTS},${SKIPHLTFAIL}\)

mkdir -p $OUTPUTDIR
mv ${DATASET}_${FILESET}_scntuple.root* $OUTPUTDIR/
